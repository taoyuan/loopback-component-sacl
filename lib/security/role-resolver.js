...
module.exports = function (sec) {
	...
	Role.registerResolver(role, (role, context) => {
		...
		const remotingInfo = remotingData ? 'with remoting data: ' + util.inspect(remotingData, {colors: true, breakLength: Infinity}) : '';
		...
		return acl.hasRoleByName(userId, 'admin').then(isAdmin => {
			...
			return Promise.all([
				...
				return Promise.resolve(current.type === sec.options.userModel && current.id === userId)
					.then(allowed => {
						if (!allowed) {
					...
					.then(allowed => {
						debug(`${mmi} User %s is %s to perform %s(%s) in group %s`,
							chalk.magenta(userId),
							chalk.magenta.bold(allowed ? 'allowed' : 'not allowed'),
							chalk.magenta(modelName + '.' + method),
							chalk.magenta(action),
							util.inspect(current, {colors: true, breakLength: Infinity})
						);
	...
	function resolveAction(Model, method) {
		const originModel = Model.modelName;
		const originMethod = method;
		const matches = method.match(regRelMethod);
		if (matches) {
			Model = _.get(Model.relations[matches[3]], 'modelTo');
			method = matches[2];
			if (!Model || !method) throw new Error(g.f('Can not parse method %s of model %s for relation %s', originMethod, originModel, matches[3]));
		}
		...
		return _.toUpper(prefix + sec.getActionForMethod(Model, method));
	}

	function getCurrentGroup(context) {
		const {rel} = sec.options;
