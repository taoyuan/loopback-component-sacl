...
module.exports = function (sec) {
	...
	function buildWhere(userId, Model, where) {
		...
		} else if (relKey = utils.getRelKey(Model, sec.options.rel)) {
			groupType = relKey.keyType || where[relKey.keyTypeWhere];
			groupKey = relKey.keyId;
		} else {
			throw new Error(g.f('ACCESS denied: Model %s has no relation %s to group', Model.modelName, sec.options.rel));
		}
