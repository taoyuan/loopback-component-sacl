...
const debug = require('debug')('loopback:component:sacl:load-abilities');
const _ = require('lodash');
const Promise = require('bluebird');

module.exports = function (sec) {
	const models = _.concat(sec.groups, sec.resources);
	...
	return Promise.map(models, model => {
		...
		return actions.length && Ability.addActions(modelName, actions).then(ability => {
				if (ability) {
					...
					debug('ailed loading actions "%s": %j', modelName, actions);
				}
			});
	}).then(() => {
